...
export async function registerUser(req: Request, res: Response): Promise<any> {
    try {
        ...
        const newUser = await connectionPool.query("INSERT INTO users (name, email, password, type) VALUES ($1, $2, $3, $4) RETURNING *", [name, email, password, type]);
...
export async function loginUser(req: Request, res: Response): Promise<any> {
    ...
    try {
        ...
        const user = await connectionPool.query("SELECT * FROM users WHERE email = $1 AND password = $2", [email, password]);
